...
{   
    ...
    private readonly LinkedList<string> _comboQueue = new();
    private readonly Dictionary<string, LinkedListNode<string>> _lookup = new();
    ...
    public void Enqueue(string id)
    {
        ...
        _lookup[id] = _comboQueue.AddLast(id);
    }

    public string Dequeue()
    {
        if (_comboQueue.Count == 0) return null;
    ...
    {
        if (!_lookup.TryGetValue(id, out var node)) return false;
    ...
    public bool Contains(string id) => _lookup.ContainsKey(id);

    public void HandleTap(string targetId)
    {
        if (Contains(targetId)) return;
    ...
    public void HandleMatch()
    {
        ...
        DOTween.Sequence()
            .Append(ComboText.DOFade(1f, 0.15f))
            .AppendInterval(0.5f)
            .Append(ComboText.DOFade(0f, 0.3f));
    }
}